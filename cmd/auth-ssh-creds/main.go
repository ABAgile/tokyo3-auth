// Command auth-ssh-creds is a credential helper that mints short-lived
// SSH user certificates from certd, gated by an OIDC ID token issued by
// auth's IdP.
//
...
// a loopback redirect, caches the refresh + ID token):
//
//	$ auth-ssh-creds login --issuer https://id.example.com --client-id tokyo3-cli
//
// On-demand cert minting (renews access/ID tokens silently if they're
// stale, generates an ed25519 keypair the first time, POSTs the public
// key to certd along with the ID token as a bearer credential, writes
// the returned cert next to the private key):
//
//	$ auth-ssh-creds get \
