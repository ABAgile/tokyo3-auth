...
// standard library — so that the auth-aws-creds CLI helper can validate
// federation tokens without pulling in auth's database backend, AWS SDK,
// audit pipeline, or any of the server-side packages. The JWT signer in
// internal/jwt and any future tooling share the constants and types
// declared here, keeping a single source of truth for the wire format.
//
