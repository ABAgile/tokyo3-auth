...
-- without violating the FK to users(id). SQLite doesn't support
-- DROP NOT NULL on an existing column, so rebuild + rename.
...
SELECT id, user_id, client_id, access_token_hash, refresh_token_hash, scopes, access_expires_at, refresh_expires_at, last_activity_at, mfa_verified, created_at
FROM sessions;
