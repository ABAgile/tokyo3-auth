...
func (s *DB) CreateGroup(ctx context.Context, displayName string) (*model.SCIMGroup, error) {
	...
	_, err := s.db.ExecContext(ctx,
		`INSERT INTO scim_groups (id, display_name, created_at, updated_at) VALUES (?, ?, ?, ?)`,
...
func (s *DB) GetGroupByID(ctx context.Context, id uuid.UUID) (*model.SCIMGroup, error) {
	...
	err := s.db.QueryRowContext(ctx,
		`SELECT id, display_name, created_at, updated_at FROM scim_groups WHERE id = ?`, id).
...
func (s *DB) UpdateGroup(ctx context.Context, id uuid.UUID, displayName string) error {
	_, err := s.db.ExecContext(ctx,
		`UPDATE scim_groups SET display_name = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?`,
		displayName, id)
	return err
}
...
func (s *DB) ReplaceGroupMembers(ctx context.Context, groupID uuid.UUID, memberIDs []uuid.UUID) error {
	...
	for _, uid := range memberIDs {
		if _, err := tx.ExecContext(ctx,
			`INSERT INTO scim_group_members (group_id, user_id) VALUES (?, ?) ON CONFLICT (group_id, user_id) DO NOTHING`,
			groupID, uid); err != nil {
			_ = tx.Rollback()
			return err
		}
...
func (s *DB) DeleteGroup(ctx context.Context, id uuid.UUID) error {
	_, err := s.db.ExecContext(ctx, `DELETE FROM scim_groups WHERE id = ?`, id)
	return err
}

func (s *DB) groupMembers(ctx context.Context, groupID uuid.UUID) ([]uuid.UUID, error) {
	rows, err := s.db.QueryContext(ctx,
		`SELECT user_id FROM scim_group_members WHERE group_id = ? ORDER BY user_id`, groupID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
