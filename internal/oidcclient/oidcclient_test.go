...
func TestConfigAndTokensRoundtrip(t *testing.T) {
	...
	if err := oidcclient.SaveConfig(app, cfg); err != nil {
		t.Fatalf("SaveConfig: %v", err)
	}
	got, err := oidcclient.LoadConfig(app)
	if err != nil {
	...
	if err := oidcclient.SaveTokens(app, tokens); err != nil {
		t.Fatalf("SaveTokens: %v", err)
	}
	gotT, err := oidcclient.LoadTokens(app)
	if err != nil {
...
func TestLogout_KeepsConfig(t *testing.T) {
	...
	const app = "test-app"
	if err := oidcclient.SaveConfig(app, oidcclient.Config{Issuer: "i", ClientID: "c"}); err != nil {
		t.Fatalf("SaveConfig: %v", err)
	}
	if err := oidcclient.SaveTokens(app, &oidcclient.Tokens{AccessToken: "a"}); err != nil {
		t.Fatalf("SaveTokens: %v", err)
	}
	...
	if err := oidcclient.Logout(app, "sub"); err != nil {
...
func TestEnsureFreshTokens_KeepsRefreshTokenWhenServerOmits(t *testing.T) {
	...
	if err := oidcclient.SaveTokens(app, &oidcclient.Tokens{
		AccessToken:  "old",
	...
	got, err := oidcclient.EnsureFreshTokens(t.Context(), app, cfg, 30*time.Second)
	if err != nil {
...
func TestEnsureFreshTokens_ReturnsCachedWhenWithinSkew(t *testing.T) {
	...
	const app = "test-app"
	cfg := oidcclient.Config{Issuer: srv.URL, ClientID: "c"}
	_ = oidcclient.SaveConfig(app, cfg)
	_ = oidcclient.SaveTokens(app, &oidcclient.Tokens{
