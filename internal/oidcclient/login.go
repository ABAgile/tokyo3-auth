...
// $XDG_CONFIG_HOME/<appName>/, and returns the Tokens.
//
...
func Login(ctx context.Context, appName string, cfg Config, opt LoginOptions) (*Tokens, error) {
	if cfg.Issuer == "" || cfg.ClientID == "" {
		return nil, errors.New("oidcclient.Login: Issuer and ClientID are required")
	}
	if err := SaveConfig(appName, cfg); err != nil {
		return nil, err
	}
	var (
		tokens *Tokens
	...
	if err := SaveTokens(appName, tokens); err != nil {
		return nil, err
	}
	return tokens, nil
}
